 ...
 * fortune-wheel/v1) — served from R2 like the other games. The `nothreads` export needs
 * no COOP/COEP headers. NEXT_PUBLIC_WHEEL_GAME_URL overrides it. Re-upload with the game's
 * web/build.sh (bump the version prefix per rebuild).
 */
const R2_GAME_URL =
  "https://pub-a2458a29274f4f5ba61f429adf2fcf8f.r2.dev/fortune-wheel/v3/index.html";
...
/**
 * Embeds the Fortune Wheel Godot/WASM client and is the server-authoritative bridge: the
 * iframe asks to place a bet (with the selected risk) over postMessage, this owns the
 * session and calls the Aureus API, and only the returned outcome is sent back for the
 * wheel to animate. The client never decides a result; the risk only selects the public
 * wheel layout — the payout is the server's.
 */
export function WheelGodot({
  game,
  currency,
}: {
  game: GameDTO;
  ...
  const post = useCallback((type: string, payload: unknown, reqId?: string) => {
    iframeRef.current?.contentWindow?.postMessage(
      { source: "wheel-host", type, payload, reqId },
      GAME_ORIGIN,
    );
  }, []);
  ...
  useEffect(() => {
    function onMessage(e: MessageEvent): void {
      ...
      if (!m || m.source !== "wheel-game") return;
  ...
  if (!GAME_URL) {
    return (
      <div className="flex aspect-video w-full items-center justify-center rounded-xl border border-hairline bg-surface-1 text-center text-sm text-text-mid">
        Fortune Wheel is loading on the game server — set NEXT_PUBLIC_WHEEL_GAME_URL.
      </div>
    );
  }

  return <GameViewport ref={iframeRef} src={GAME_URL} title="Fortune Wheel" onLoad={sendInit} />;
}
