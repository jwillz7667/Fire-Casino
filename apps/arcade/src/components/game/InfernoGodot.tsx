 ...
 * inferno-link/v1) — served from R2 like the other games. The `nothreads` export needs no
 * COOP/COEP headers. NEXT_PUBLIC_INFERNO_GAME_URL overrides it. Re-upload with the game's
 * web/build.sh (bump the version prefix per rebuild).
 */
const R2_GAME_URL =
  "https://pub-a2458a29274f4f5ba61f429adf2fcf8f.r2.dev/inferno-link/v4/index.html";
...
/**
 * Embeds the Inferno Link Godot/WASM client and is the server-authoritative bridge: the
 * iframe asks to place a bet over postMessage, this owns the session and calls the Aureus
 * API, and only the returned outcome (base grid, line wins, hold-and-spin) is sent back for
 * the client to animate. The client never decides a result.
 */
export function InfernoGodot({
  game,
  currency,
}: {
  game: GameDTO;
  ...
  const post = useCallback((type: string, payload: unknown, reqId?: string) => {
    iframeRef.current?.contentWindow?.postMessage(
      { source: "inferno-host", type, payload, reqId },
      GAME_ORIGIN,
    );
  }, []);
  ...
  useEffect(() => {
    function onMessage(e: MessageEvent): void {
      ...
      if (!m || m.source !== "inferno-game") return;
  ...
  if (!GAME_URL) {
    return (
      <div className="flex aspect-video w-full items-center justify-center rounded-xl border border-hairline bg-surface-1 text-center text-sm text-text-mid">
        Inferno Link is loading on the game server — set NEXT_PUBLIC_INFERNO_GAME_URL.
      </div>
    );
  }
