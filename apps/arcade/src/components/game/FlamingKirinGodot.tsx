 ...
 * — served from R2 like Cosmic/Inferno (too large to bundle into the Vercel deploy). The
 * `nothreads` export needs no COOP/COEP headers. NEXT_PUBLIC_KIRIN_GAME_URL overrides it.
 * Re-upload with games/flaming-kirin/web/build.sh (bump the version prefix per rebuild).
 */
const R2_GAME_URL =
  "https://pub-a2458a29274f4f5ba61f429adf2fcf8f.r2.dev/flaming-kirin/v4/index.html";
...
/**
 * Embeds the Legend of the Flaming Kirin Godot/WASM client and is the server-authoritative
 * bridge: the iframe asks to place a bet over postMessage, this owns the session and calls the
 * Aureus API, and only the returned outcome is sent back for the game to animate. The client
 * never decides a result. Balance + wallet cache stay in sync.
 */
export function FlamingKirinGodot({
  game,
  currency,
}: {
  game: GameDTO;
  ...
  const post = useCallback((type: string, payload: unknown, reqId?: string) => {
    iframeRef.current?.contentWindow?.postMessage(
      { source: "flaming-kirin-host", type, payload, reqId },
      GAME_ORIGIN,
    );
  }, []);
  ...
  useEffect(() => {
    function onMessage(e: MessageEvent): void {
      ...
      if (!m || m.source !== "flaming-kirin-game") return;
  ...
  if (!GAME_URL) {
    return (
      <div className="flex aspect-video w-full items-center justify-center rounded-xl border border-hairline bg-surface-1 text-center text-sm text-text-mid">
        Legend of the Flaming Kirin is loading on the game server — set NEXT_PUBLIC_KIRIN_GAME_URL.
      </div>
    );
  }

  return (
    <GameViewport ref={iframeRef} src={GAME_URL} title="Legend of the Flaming Kirin" onLoad={sendInit} />
  );
}
