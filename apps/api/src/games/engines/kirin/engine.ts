...
export interface SpinResult {
  ...
  bonusCount: number;
  bonusPayBps: number;
  multiplier: number; // sticky free-spins "Kirin Fire" multiplier (1 in base)
...
export interface KirinOutcome extends Record<string, unknown> {
  kind: "flaming-kirin";
  win: boolean;
  base: SpinResult;
...
function weightedSymbol(rng: Rng, weights: Record<SymbolId, number>): SymbolId {
  ...
  return "J"; // unreachable given positive total; satisfies the type
}
...
/**
 * Left-aligned payline wins with wild substitution. The WILD only lands on interior reels, so
 * reel 1 is always a real symbol and fixes the line's paying symbol; a WILD on any later reel
 * substitutes to extend the run. Each line pays its single left-aligned k-of-a-kind once, and
 * only if the paytable entry is non-zero.
 */
...
/**
 * Evaluate one spin: payline wins (wild-substituted) + scatter pay (× multiplier), plus the
 * BONUS count and its FIXED instant prize. The bonus prize is reported but NOT folded into
 * `spinWinBps` (which carries only the RTP-scaled line+scatter slice); the round adds the
 * unscaled bonus on top.
 */
...
function runFreeSpins(rng: Rng, triggerScatters: number): FreeSpinsResult {
  ...
  while (remaining > 0) {
    ...
    // Deterministic +1-per-spin multiplier (1,2,3,…) capped. Consumes no RNG, so the draw
    // order stays a pure function of (reel, row).
...
/**
 * Roll the four-tier jackpot. One RNG draw decides whether it strikes (so it never perturbs
 * the grid draw order); a second weighted draw picks the tier. Returns the struck tier + its
 * fixed bet-multiple award (verbatim, unscaled), or null.
 */
function rollJackpot(rng: Rng): JackpotResult | null {
  if (rng() >= JACKPOT_CHANCE) return null;
  const tiers = Object.keys(JACKPOT_TIER_WEIGHTS) as KirinJackpotTier[];
  let total = 0;
  for (const t of tiers) total += JACKPOT_TIER_WEIGHTS[t];
  let r = rng() * total;
  let tier: KirinJackpotTier = "MINI";
  for (const t of tiers) {
    r -= JACKPOT_TIER_WEIGHTS[t];
    if (r < 0) {
      tier = t;
      break;
    }
  }
  return { tier, awardBps: JACKPOT_AWARD[tier] };
}

/**
 * Run one full Flaming Kirin round on a provable-fairness RNG. Pure: identical RNG ⇒ identical
 * outcome. Returns the win in bps of total bet (the provider turns it into integer minor units)
 * plus the full render payload.
 *
 * Calibration model: the line + scatter + free-spin slice is scaled by PAYOUT_SCALAR_BPS; the
 * BONUS prize and the JACKPOT are added VERBATIM (never scaled) so their reveals are exact. The
 * 5000× cap binds last.
 */
export function spin(rng: Rng): EngineResult {
  ...
  const jackpot = rollJackpot(rng);

  const rawScaledBps = base.spinWinBps + (freeSpins?.totalBps ?? 0);
  const scaled = Math.floor((rawScaledBps * PAYOUT_SCALAR_BPS) / 10_000);
  // The instant bonus prize and the jackpot ride on top of the scaled slice, unscaled.
  const totalWinBps = Math.min(
    MAX_WIN_BPS,
