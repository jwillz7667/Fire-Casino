...
/**
 * Monte-Carlo RTP / behaviour probe for the Dragon's Hoard Bonanza engine. Run ad hoc:
 *   pnpm --filter api exec tsx src/games/engines/dragon/simulate.ts [spins]
 * Uses a seeded PRNG (not the provable-fairness stream) purely to measure the model's
 * intrinsic RTP, hit rate, free-spin frequency and tail. The certified RTP pinned in
 * math.ts / the catalog comes from this measurement.
 */
...
// RTP is linear in PAYOUT_SCALAR_BPS, so the scalar that hits the certified target
// exactly is one re-run away.
