...
import { isPaying, SCATTER, WILD, type SymbolId } from "./symbols";
...
export interface DragonOutcome extends Record<string, unknown> {
  kind: "dragon-hoard";
  win: boolean;
  base: SpinResult;
...
function lineWins(grid: Grid): LineWin[] {
  ...
  for (let line = 0; line < PAYLINES.length; line++) {
    ...
    if (!isPaying(target)) continue; // scatter-led (or otherwise non-paying) line
    ...
    if (run >= 3) {
      ...
      // Filler royals pay 0 at 3 — don't push a phantom 0-pay win line.
...
/** Evaluate one spin: payline wins (wild-substituted) + scatter pay, times multiplier. */
...
function runFreeSpins(rng: Rng, triggerScatters: number): FreeSpinsResult {
  ...
  while (remaining > 0) {
    ...
    // Deterministic +1-per-spin hoard multiplier (1,2,3,…) capped. Consumes no RNG, so
    // the draw order stays a pure function of (reel, row).
...
/**
 * Run one full Dragon's Hoard Bonanza round on a provable-fairness RNG. Pure: identical
 * RNG ⇒ identical outcome. Returns the win in bps of total bet (the provider turns it
 * into integer minor units) plus the full render payload.
 */
export function spin(rng: Rng): EngineResult {
  ...
  // reel-by-reel (free spins auto-play). The COINS scatter triggers free spins on 3+ and teases
