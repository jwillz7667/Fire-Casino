// Dragon's Hoard Bonanza — iframe-side bridge.
//
// Embedded in the arcade iframe (cross-origin: game on the arcade origin's static
// host, parent owns auth/session/API): proxies window.DragonGodot.getInit()/placeBet()
// to the parent over postMessage so every outcome stays server-authoritative.
//
// Opened standalone (not in an iframe): leaves window.DragonGodot undefined, so the
// Godot client falls back to its built-in offline demo (mock outcomes). That makes the
// exported build directly loadable for QA without a host.
(function () {
  "use strict";
  ...
  function send(type, payload, reqId) {
    parent.postMessage({ source: "dragon-game", type: type, payload: payload, reqId: reqId }, "*");
  }
  ...
  window.addEventListener("message", function (e) {
    var m = e.data;
    if (!m || m.source !== "dragon-host") return;
    if (m.type === "init") {
      init = m.payload;
  ...
  // Ask the host for initial state as soon as the shell is parsed; the host also pushes
  // it on iframe load, so it is cached well before Godot boots.
  send("requestInit", {});
})();
