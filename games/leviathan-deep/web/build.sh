#!/usr/bin/env bash
# Leviathan's Deep — export the Godot web build, inject the bridge, and upload it to public
# Cloudflare R2 (bucket `goldwave`, prefix leviathan-deep/<VER>). The build AND the lobby
# thumbnail are served from R2 (NOT bundled into the Vercel deploy). The `nothreads` preset needs
# no COOP/COEP headers, so the plain r2.dev host serves it.
#
# Re-run after any client change and BUMP VER on a rebuild (R2 objects are immutable-cached), then
# point R2_GAME_URL in the arcade host component (apps/arcade/.../LeviathanGodot.tsx) at the new
# prefix.
#
...
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"   # games/leviathan-deep
REPO="$(cd "$HERE/../.." && pwd)"
...
cp "$HERE/web/leviathan-bridge.js" "$HERE/build/leviathan-bridge.js"
...
DEST="s3://${R2_BUCKET_ASSETS}/leviathan-deep/${VER}"
ct() { case "$1" in
...
if [ -f "$HERE/thumb.png" ]; then
  aws s3 cp "$HERE/thumb.png" "s3://${R2_BUCKET_ASSETS}/leviathan-deep/thumb.png" \
    --endpoint-url "$ENDPOINT" --content-type image/png \
    --cache-control "public,max-age=86400" --no-progress
  echo "thumbnail -> s3://${R2_BUCKET_ASSETS}/leviathan-deep/thumb.png"
fi

echo "uploaded -> $DEST"
echo "set R2_GAME_URL in the arcade LeviathanGodot host to the new ${VER}/index.html prefix"
