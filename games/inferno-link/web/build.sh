#!/usr/bin/env bash
# Inferno Link — export the Godot web build, inject the bridge, and upload to public
# Cloudflare R2 (bucket `goldwave`, prefix inferno-link/<VER>). Served from R2 like the
...
# then point R2_GAME_URL in apps/arcade/src/components/game/InfernoGodot.tsx at it.
#
...
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"   # games/inferno-link
REPO="$(cd "$HERE/../.." && pwd)"
...
cp "$HERE/web/inferno-bridge.js" "$HERE/build/inferno-bridge.js"
...
DEST="s3://${R2_BUCKET_ASSETS}/inferno-link/${VER}"
ct() { case "$1" in
...
echo "uploaded -> $DEST"
echo "set R2_GAME_URL in InfernoGodot.tsx to https://pub-a2458a29274f4f5ba61f429adf2fcf8f.r2.dev/inferno-link/${VER}/index.html"
