// Plinko — iframe-side bridge.
//
// Embedded in the arcade iframe (cross-origin: game on R2, parent owns auth/session/API):
// proxies window.PlinkoGodot.getInit()/placeBet() to the parent over postMessage so every
// outcome stays server-authoritative. placeBet carries per-bet params (the selected risk).
//
// Opened standalone (not in an iframe): leaves window.PlinkoGodot undefined, so the Godot
...
(function () {
  ...
  function send(type, payload, reqId) {
    ...
    parent.postMessage({ source: "plinko-game", type: type, payload: payload, reqId: reqId }, hostOrigin || "*");
  }
  ...
  window.addEventListener("message", function (e) {
    var m = e.data;
    if (!m || m.source !== "plinko-host") return;
