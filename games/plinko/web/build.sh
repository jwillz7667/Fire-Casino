#!/usr/bin/env bash
# Plinko — export the Godot web build, inject the bridge, and upload to public Cloudflare R2
# (bucket `goldwave`, prefix plinko/<VER>). Served from R2 like the other games. The
# `nothreads` preset needs no COOP/COEP headers.
#
# Re-run after any client change and BUMP VER on a rebuild (R2 objects are immutable-cached),
# then point R2_GAME_URL in apps/arcade/src/components/game/PlinkoGodot.tsx at it.
#
...
set -euo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"   # games/plinko
REPO="$(cd "$HERE/../.." && pwd)"
...
cp "$HERE/web/plinko-bridge.js" "$HERE/build/plinko-bridge.js"
...
DEST="s3://${R2_BUCKET_ASSETS}/plinko/${VER}"
ct() { case "$1" in
...
echo "uploaded -> $DEST"
echo "set R2_GAME_URL in PlinkoGodot.tsx to https://pub-a2458a29274f4f5ba61f429adf2fcf8f.r2.dev/plinko/${VER}/index.html"
